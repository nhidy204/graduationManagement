        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div>
            ...
            <div className="mt-2 flex items-end gap-3">
              <span className="text-6xl font-bold">{final.toFixed(2)}</span>
          ...
          <div className="flex flex-col gap-2 min-w-[180px]">
            <ScoreRow label="Điểm hướng dẫn (60%)" value={result.supervisorScore!} weighted={result.supervisorScore! * 0.6} />
            <div className="h-px bg-white/20" />
            <ScoreRow label="Điểm phản biện (40%)" value={result.reviewerScore!} weighted={result.reviewerScore! * 0.4} />
            <div className="h-px bg-white/20" />
            <ScoreRow label="Tổng" value={final} weighted={null} bold />
          </div>
        </div>
      </div>
...
function ScoreRow({ label, value, weighted, bold = false }: {
  label: string; value: number; weighted: number | null; bold?: boolean;
}) {
  return (
    <div className={cn('flex items-center justify-between gap-4', bold && 'font-bold')}>
      ...
      <span className={cn('text-sm text-white tabular-nums', bold && 'text-lg')}>
        {weighted !== null ? `${value.toFixed(1)} → ${weighted.toFixed(2)}` : value.toFixed(2)}
      </span>
    </div>
  );
}
...
function DetailSection({ title, score, criteria, type, expanded, onToggle }: {
  title: string; score: number; criteria: GradeCriteria[];
  type: 'supervisor' | 'reviewer'; expanded: boolean; onToggle: () => void;
}) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
      <button onClick={onToggle} className="flex w-full items-center justify-between p-4 hover:bg-gray-50 transition-colors">
        <div className="flex items-center gap-3">
          <div className={cn('flex h-8 w-8 items-center justify-center rounded-lg', type === 'supervisor' ? 'bg-blue-100' : 'bg-purple-100')}>
            <Star className={cn('h-4 w-4', type === 'supervisor' ? 'text-blue-600' : 'text-purple-600')} />
          </div>
          <div className="text-left">
            <p className="text-sm font-semibold text-gray-900">{title}</p>
            <p className={cn('text-xs font-medium', type === 'supervisor' ? 'text-blue-600' : 'text-purple-600')}>
              {score.toFixed(2)}/10
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="hidden w-24 md:block">
            <div className="h-1.5 w-full rounded-full bg-gray-100">
              <div className={cn('h-1.5 rounded-full transition-all', type === 'supervisor' ? 'bg-blue-500' : 'bg-purple-500')}
      ...
      {expanded && (
        <div className="border-t border-gray-100 p-4">
          <div className="flex flex-col gap-2">
              ...
              <div key={c._id} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-gray-200 px-1.5 py-0.5 text-xs text-gray-500">{Math.round(c.weight * 100)}%</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5">{c.description}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0 ml-4">
                  <div className="w-16 h-1.5 rounded-full bg-gray-200">
                    <div className={cn('h-1.5 rounded-full', type === 'supervisor' ? 'bg-blue-500' : 'bg-purple-500')}
            ...
            <div className={cn('flex items-center justify-between rounded-xl p-3 mt-1', type === 'supervisor' ? 'bg-blue-50' : 'bg-purple-50')}>
              <p className={cn('text-sm font-semibold', type === 'supervisor' ? 'text-blue-800' : 'text-purple-800')}>Tổng điểm</p>
              <p className={cn('text-lg font-bold', type === 'supervisor' ? 'text-blue-700' : 'text-purple-700')}>{score.toFixed(2)}/10</p>
            </div>
          </div>
        </div>
      )}
